                    ...
                    <div className='min-h-[70vh] w-full'>
                        <TabsContent value="experience" className="w-full ">
                            <div className='flex flex-col gap-[30px] text-center lg:text-left'>
                                ...
                                <p className='max-w-[600px] text-black dark:text-white/60 mx-auto lg:mx-0 '>{experience.description}</p>
                            </div>
                        </TabsContent>
                        ...
                        <TabsContent value="education" className="w-full ">
                            <div className='flex flex-col gap-[30px] text-center lg:text-left'>
                                ...
                                <p className='max-w-[600px] text-black dark:text-white/60 mx-auto lg:mx-0 '>{education.description}</p>
                            </div>
                            <ScrollArea className="h-[400px] mt-[30px]">
                                <ul className='grid grid-col-1 lg:grid-col-2 gap-[30px]'>
                                    {
                                        education.items.map((item, index) => {
                                            return <li key={index} className='bg-[#a4a4aa9f] dark:bg-[#232329] h-[184px] py-6 px-10 rounded-xl flex flex-col justify-center items-center lg:items-start gap-1 '>
                                                <span className='text-accent'>{item.duration}</span>
                                                <h3 className='text-xl max-w-[260px] min-h-[60px] text-center lg:text-left'>{item.degree}</h3>
                                                <div className='flex items-center gap-3'>
                                                    <span className='w-[6px] h-[6px] rounded-full bg-accent'></span>
                                                    <p className='text-black dark:text-white/60'>{item.institution}</p>
                                                </div>
                                            </li>
                                        })
                                    }
                        ...
                        <TabsContent value="skills" className="w-full">
                            <div className='flex flex-col gap-[30px]'>
                                <div className='flex flex-col gap-[30px] text-center lg:text-left'>
                                    ...
                                    <p className='max-w-[600px] text-black dark:text-white/60 mx-auto lg:mx-0'>{skills.description}</p>
                                </div>
                            </div>
                            <ul className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 lg:gap-[30px] mt-[30px]'>
                                {skills.skillList.map((skill, index) => {
                                    return <li className='w-[170px]' key={index}>
                                        <TooltipProvider delayDuration={100}>
                                            <Tooltip>
                                                <TooltipTrigger className='w-full h-[150px] bg-[#a4a4aa9f] dark:bg-[#232329] rounded-xl flex justify-center items-center group'>
                        ...
                        <TabsContent value="about" className="w-full text-center lg:text-left">
                            <div className='flex flex-col gap-[30px]'>
                                ...
                                <p className='max-w-[600px] text-black dark:text-white/60 mx-auto lg:mx-0'>{about.description}</p>
                                <ul className='grid grid-cols-1 gay-y-6 max-w-[620px] mx-auto lg:mx-0'>
                                    {
                                        about.info.map((item, index) => {
                                            return <li key={index} className='flex items-center lg:justify-start gap-4'>
                                                <span className='text-black dark:text-white/60'>{item.fieldName}</span>
